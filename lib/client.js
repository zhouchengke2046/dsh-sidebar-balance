	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + CSS_ID + '"]') === null) {
			...
			tag.textContent = [
				...
				"@keyframes psb-pop{from{opacity:0;scale:.96}to{opacity:1;scale:1}}",
				...
				".psb-fill{height:100%;border-radius:3px;box-shadow:inset 0 1px 0 rgba(255,255,255,.22);transition:width .4s ease}",
		...
		// ── i18n dictionaries ──────────────────────────────────────────────
		const NS = "dsh-sidebar-balance";
		const zh = {
			"settings": "设置",
			"balance": "余额",
			"account.overview": "账户概览",
			"refresh": "刷新",
			"ds.account": "DeepSeek 账户",
			"available": "可用",
			"unavailable": "不可用",
			"topup": "充值",
			"grant": "赠送",
			"balance.fail": "获取最新余额失败，请稍后再试",
			"loading": "加载中…",
			"go.plan": "OpenCode Go 套餐",
			"go.rolling": "滚动 (5h)",
			"go.weekly": "周",
			"go.monthly": "月",
			"go.status": "状态",
			"go.noData": t("go.noData"),
			"go.fail": "获取 OpenCode Go 套餐用量失败，请稍后再试",
			"go.missingKey": t("go.missingKey"),
			"go.ringMissing": "未配置 OpenCode Go key(设置 → 模型)",
			"go.ringLoading": "OpenCode Go 套餐加载中…",
			"go.ringUsed": "OpenCode Go 套餐已用",
			"resets.soon": "即将重置",
			"resets.min": "{n}分后重置",
			"resets.hour": "{n}小时{m}分后重置",
			"resets.day": "{n}天{h}小时后重置"
		};
		const en = {
			"settings": "Settings",
			"balance": "Balance",
			...
			"refresh": "Refresh",
			"ds.account": "DeepSeek account",
			"available": "Available",
			"unavailable": "Unavailable",
			"topup": "Top-up",
			"grant": "Grant",
			"balance.fail": "Could not refresh the latest balance, try again later",
			"loading": "Loading…",
			"go.plan": "OpenCode Go plan",
			"go.rolling": "Rolling (5h)",
			"go.weekly": "Weekly",
			"go.monthly": "Monthly",
			"go.status": "Status",
			"go.noData": "No window data",
			"go.fail": "Could not refresh OpenCode Go usage, try again later",
			"go.missingKey": "OpenCode Go key not configured (set OPENCODE_GO_API_KEY in Settings → Models)",
			"go.ringMissing": "OpenCode Go key not configured (Settings → Models)",
			"go.ringLoading": "OpenCode Go usage loading…",
			"go.ringUsed": "OpenCode Go plan used",
			"resets.soon": "resets soon",
			"resets.min": "resets in {n}m",
			"resets.hour": "resets in {n}h {m}m",
			"resets.day": "resets in {n}d {h}h"
		};
		/** Minimal translate; falls back to zh when the seat provides none. */
		function makeT(seatT) {
		...
		function timeUntil(iso, t) {
			...
			if (diff <= 0) return t("resets.soon");
			const mins = Math.floor(diff / 60000);
			if (mins < 60) return t("resets.min", { n: mins });
			const hours = Math.floor(mins / 60);
			if (hours < 24) return t("resets.hour", { n: hours, m: mins % 60 });
			return t("resets.day", { n: Math.floor(hours / 24), h: hours % 24 });
		}
		...
		// Self-scheduling poller with exponential backoff: success keeps a
		// steady baseMs cadence; consecutive failures back off 20s→40s→… capped
		// at 5min, so a flaky key/network never hammers the upstream.
		function useAdaptivePoll(fn, baseMs, deps) {
			...
			react.useEffect(() => {
				...
				let timer = 0;
				let fails = 0;
				const nextDelay = (ok) => (ok ? baseMs : Math.min(20000 * Math.pow(2, Math.max(0, fails - 1)), 300000));
				const tick = async () => {
					if (!alive) return;
					// Pause polling while the tab is hidden (saves requests); a
					// visibilitychange back to visible runs an immediate tick.
					if (typeof document !== "undefined" && document.visibilityState === "hidden") {
						timer = window.setTimeout(tick, baseMs);
						return;
					}
					let ok = false;
					try { ok = (await fnRef.current()) === true; } catch { ok = false; }
					fails = ok ? 0 : fails + 1;
					timer = window.setTimeout(tick, nextDelay(ok));
				};
				const onVisible = () => {
		...
		function GoRing({ percent, title }) {
			...
			return react_jsx_runtime.jsx("svg", {
				...
				title: title,
				role: "img",
				"aria-label": title || "OpenCode Go 套餐用量",
				children: [
		...
		function TriggerWithBalance(props) {
			...
			// Poll the DeepSeek balance (adaptive backoff; re-run on stamp).
			const balLoad = async () => {
				setBalLoading(true);
				let ok = false;
				try {
			...
			useAdaptivePoll(balLoad, 60000, [stamp]);
			...
			// Keep the Go consumption ring fresh: load on mount + adaptive poll.
			useAdaptivePoll(loadGo, 60000, []);

			const openTip = (e) => {
				if (hideTimer.current) { window.clearTimeout(hideTimer.current); hideTimer.current = null; }
				const rect = e.currentTarget.getBoundingClientRect();
				const estW = 300, estH = 250, m = 8;
				const vw = window.innerWidth, vh = window.innerHeight;
				const below = rect.top < estH + m;
				let left = rect.left;
				if (left + estW > vw - m) left = vw - m - estW;
				left = Math.max(m, Math.min(left, vw - m));
				setTip({ x: Math.round(left), y: Math.round(below ? rect.bottom + m : rect.top - m), below });
			...
			const goMissingKey = !!go.error && go.error.indexOf("未配置") !== -1;
			...
			if (b) {
				...
				dsCard = react_jsx_runtime.jsx("div", { className: "psb-ds-card", children: [
					react_jsx_runtime.jsx("div", { className: "psb-ds-top", children: [
						react_jsx_runtime.jsx("span", { className: "psb-ds-lbl", children: t("ds.account") }),
						react_jsx_runtime.jsx("span", { className: "psb-ds-status" + (b.is_available ? "" : " bad"), children: b.is_available ? t("available") : t("unavailable") })
					] }),
					react_jsx_runtime.jsx("div", { className: "psb-ds-amt", children: [
					react_jsx_runtime.jsx("span", { className: "psb-dot", "aria-hidden": true, style: { background: dotColor } }),
					react_jsx_runtime.jsx("span", { className: "sym", "aria-hidden": true, children: sym }),
						react_jsx_runtime.jsx(SwapText, { tick: animTick, value: amount ?? "—" })
					] }),
					react_jsx_runtime.jsx("div", { className: "psb-ds-sub", children: [
						react_jsx_runtime.jsx("span", { children: [t("topup") + " ", react_jsx_runtime.jsx("b", { children: react_jsx_runtime.jsx(SwapText, { tick: animTick, value: sym + (b.topped_up_balance ?? "—") }) })] }),
						react_jsx_runtime.jsx("span", { children: [t("grant") + " ", react_jsx_runtime.jsx("b", { children: react_jsx_runtime.jsx(SwapText, { tick: animTick, value: sym + (b.granted_balance ?? "—") }) })] })
					] })
				] });
				if (bal.error) {
					dsCard = react_jsx_runtime.jsx(react.Fragment, { children: [
						dsCard,
						react_jsx_runtime.jsx("div", { className: "psb-err", children: t("balance.fail") + (bal.error ? " (" + bal.error + ")" : "") })
					] });
				}
			} else if (bal.error) {
				dsCard = react_jsx_runtime.jsx("div", { className: "psb-err", children: "获取最新余额失败，请稍后再试" + (bal.error ? " (" + bal.error + ")" : "") });
			} else {
				dsCard = react_jsx_runtime.jsx("div", { className: "psb-err", children: t("loading") });
			}

			let goSection = null;
			if (go.data) {
				const defs = [
					{ key: "rolling", label: t("go.rolling") },
					{ key: "weekly", label: t("go.weekly") },
					{ key: "monthly", label: t("go.monthly") }
				];
				const rows = [];
				for (const d of defs) {
						...
						children: [
							...
							react_jsx_runtime.jsx("div", {
								...
								children: w.status === "ok"
									? (w.resetsAt ? timeUntil(w.resetsAt, t) : "")
									: (t("go.status") + ": " + w.status)
							})
						]
					}, d.key));
				}
				...
				if (go.error) {
					goSection = react_jsx_runtime.jsx(react.Fragment, { children: [
						goSection,
						react_jsx_runtime.jsx("div", { className: "psb-err", children: t("go.fail") + (go.error ? " (" + go.error + ")" : "") })
					] });
				} else if (go.loading) {
					goSection = react_jsx_runtime.jsx(react.Fragment, { children: [
						goSection,
						react_jsx_runtime.jsx("div", { className: "psb-err", children: t("loading") })
					] });
				}
			} else if (go.error && goMissingKey) {
				goSection = react_jsx_runtime.jsx("div", { className: "psb-err", children: "未配置 OpenCode Go key(在 设置 → 模型 填写 OPENCODE_GO_API_KEY)" });
			} else if (go.error) {
				goSection = react_jsx_runtime.jsx("div", { className: "psb-err", children: "获取 OpenCode Go 套餐用量失败，请稍后再试" + (go.error ? " (" + go.error + ")" : "") });
			} else {
				goSection = react_jsx_runtime.jsx("div", { className: "psb-err", children: t("loading") });
			}
			...
			const tipEl = tip === null ? null : react_jsx_runtime.jsx("div", {
				...
				children: [
						...
						children: [
							react_jsx_runtime.jsx("span", {
								...
								children: [
									...
									react_jsx_runtime.jsx("span", { children: t("account.overview") })
								]
							}),
					...
					dsCard,
					react_jsx_runtime.jsx("div", { className: "psb-sec-title", children: t("go.plan") }),
					goSection
				]
			});
			...
			return react_jsx_runtime.jsx(react.Fragment, { children: [
				react_jsx_runtime.jsx("div", {
					...
					children: [
						...
						react_jsx_runtime.jsx("span", {
							className: "psb-left",
							children: [react_jsx_runtime.jsx(GearIcon, {}), react_jsx_runtime.jsx("span", { className: "psb-label", children: t("settings") })]
						}),
						...
						react_jsx_runtime.jsx("span", {
							className: "psb-amt",
							"aria-label": t("ds.account") + " " + t("balance") + " " + (amountText || "…"),
							onMouseEnter: openTip,
							onMouseLeave: closeTip,
							onClick: (e) => {
								e.stopPropagation();
							...
							children: [
								react_jsx_runtime.jsx("span", { className: "psb-dot" + (loading ? " loading" : ""), "aria-hidden": true, style: { background: dotColor } }),
								...
								react_jsx_runtime.jsx(GoRing, {
									...
									title: goMissingKey
										...
										: (goPercent === null
											? t("go.ringLoading")
											: (t("go.ringUsed") + " " + Math.round(goPercent) + "%"))
								})
							]
						})
					]
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-sidebar-balance: dictionaries");
			ctx.effect(() => ctx.slots.register({
				name: "settings.trigger",
				id: "dsh-sidebar-balance",
				priority: -1,
				label: () => ctx.locale.bind(NS)("settings"),
				locale: NS
